module sys_ctrl #(
    ...
    parameter RF_ADDRESS = 4,
    parameter ALU_FUNC_SIZE = 4
) (
    input CLK, // Clock Signal
    input RST, // Active Low Reset
    input [OPERAND_SIZE-1 : 0] ALU_OUT, // ALU Output
    input OUT_Valid, // ALU Output Valid signal

    output reg [ALU_FUNC_SIZE-1 : 0] ALU_FUN, // ALU Function select
    output reg EN, // ALU Enable signal
    output reg CLK_EN, // Clock Gating Enable signal
    output reg [RF_ADDRESS-1 : 0] Address, // Register File Address
    output reg WrEn, // Register File Write Enable signal
    output reg RdEn, // Register File Read Enable signal
    output reg [RF_WIDTH-1 : 0] WrData, // Register File Write Data Bus

    input [RF_WIDTH-1 : 0] RdData, // Register File Read Data Bus
    input RdData_Valid, // Register File Read Data Valid signal
    input [RF_WIDTH-1 : 0] RX_P_DATA, // RX Parallel Data from UART
    input RX_D_VLD, // RX Data Valid signal from UART
    input FIFO_FULL,

    output reg [RF_WIDTH-1 : 0] TX_P_DATA, // TX Parallel Data to UART
    output reg TX_D_VLD, // TX Data Valid signal to UART
    ...
    reg [RF_ADDRESS-1 : 0] Address_comb;
    reg [RF_ADDRESS-1 : 0] Address_reg;
